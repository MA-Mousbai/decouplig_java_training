...
public class Simulation {
    ...
    private boolean nextRound() {
        ...
        logger.log("Le joueur a entré " + nextGuess);

        if (nextGuess == numberToGuess) {
            logger.log("Le joueur a trouvé le bon nombre.");
            return true;
        } else if (nextGuess > numberToGuess) {
            System.out.println("Coulé! Le nombre est plus petit que " + nextGuess);
        } else {
            System.out.println("Coulé! Le nombre est plus grand que " + nextGuess);
        }
